...
## 2026-03-09 One-shot Prototype Upgrade (wave survival + growth)
- Scope:
  - Convert current shard-clear loop into timed wave survival with mandatory between-wave growth choices.
  - Keep existing rendering/input foundation; avoid full rewrite.
- Implemented (in progress):
  - `index.html`
    - Updated mission text for wave survival loop.
    - Added upgrade selection container (`#upgrade-options`) and hint (`#menu-upgrade-hint`).
  - `styles.css`
    - Added upgrade card styles, selected/locked states, and hidden utility class.
    - Added `menu` visual state for upgrade phase and start-button hide class.
  - `game.js`
    - Added wave timer and pacing fields (`waveDuration`, `waveTimer`, dynamic enemy/reward spawn timers).
    - Added tech economy (`tech`, `waveTech`) and build stats.
    - Added 3-option upgrade generation and mandatory selection flow (`upgrade` mode).
    - Reworked wave clear from "collect N shards" to "survive wave timer".
    - Added dynamic reinforcement spawn + dynamic reward core spawn during wave.
    - Upgrades now impact next-wave performance (speed/cooldown/radius/maxHP/pickup range/damage).
    - Added upgrade click + `1/2/3` hotkeys; preserved `P` pause, `R` restart, `F` fullscreen.
    - Expanded `render_game_to_text` and debug hooks for wave/upgrade/build state.
- Pending in this round:
  - Real-play + automated validation (input, wave loop, growth effect, randomness fairness, regressions).
  - Fix first-pass issues found in tests.
- Balance tuning after first pass:
  - First natural-play probe showed Wave 1 pressure too high (automation died before break).
  - Tuned to improve first-loop readability/survivability:
    - `WAVE_BASE_DURATION` `22 -> 18`
    - `BASE_ENEMY_TARGET` `3 -> 2`
    - `LEVEL_ENTRY_GRACE` `1.0 -> 1.25`
    - Slower reinforcement cadence via `getEnemySpawnInterval()`
    - Entry shield floor raised (`0.58 -> 0.9`)
- Validation (all real Playwright runs on final build):
  - Input regression: `output/web-game/v4-wave-proto/input-regression/probe.json` -> `passed=true`
    - Confirms: WASD + Arrows movement, `A` no pause, `P` pause/resume, `R` restart, `F` fullscreen, blur input clear.
  - Wave growth flow: `output/web-game/v4-wave-proto/wave-growth/probe.json` -> `passed=true`
    - Confirms: start run, enter upgrade each wave, choose upgrades, reach wave 4, growth impacts stats, stage rule visible.
  - Natural (no-force) wave clear: `output/web-game/v4-wave-proto/natural-wave-clear/probe.json` -> `passed=true`
    - Confirms: real play survives to wave break and enters upgrade mode with 3 choices.
  - Upgrade menu visibility: `output/web-game/v4-wave-proto/upgrade-menu/probe.json` -> `passed=true`
  - Randomness/fairness: `output/web-game/v4-wave-proto/randomness/probe.json`
    - `fairnessPassRate=1`, variation/replayability/readability all `pass`.
  - Lose + restart reset: `output/web-game/v4-wave-proto/lose-restart/probe.json` -> `passed=true`
  - Client action regressions:
    - `output/web-game/v4-wave-proto/movement_collect/state-0.json` (playing path OK)
    - `output/web-game/v4-wave-proto/lose_state/state-0.json` (lose path OK)
  - Console/runtime errors:
    - No `errors-*.json` found under `output/web-game/v4-wave-proto`.
